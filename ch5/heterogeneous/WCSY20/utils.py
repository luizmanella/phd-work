...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, projector=None):
    ...
    if dataset in ('mnist', 'cifar10'):
        if dataset == 'mnist':
            ...
            transform_train = transforms.Compose([
                ...
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
                *([] if projector is None else [transforms.Lambda(lambda img: projector(img.unsqueeze(0)).squeeze(0))]),
                normalize,
                ])
            # data prep for test set
            transform_test = transforms.Compose([
