...
def local_train(nets, args, net_dataidx_map, device="cpu"):
    ...
    for net_id, net in nets.items():
        if args.retrain:
            ...
            train_dl_local, test_dl_local = get_dataloader(args.dataset, args_datadir, args.batch_size, 32, dataidxs, projector=projector)
            train_dl_global, test_dl_global = get_dataloader(args.dataset, args_datadir, args.batch_size, 32, projector=projector)
...
if __name__ == "__main__":
    ...
    X_train, _, _, _ = load_cifar10_data(args_datadir)          # utils helper:contentReference[oaicite:4]{index=4}
    local_bcs  = [local_barycenter(X_train[idx])                # per-client barycenter
                for idx in net_dataidx_map.values()]
    global_bc  = global_barycenter(local_bcs)                   # server barycenter
    projector  = OTProjector(global_bc, n_samples=512, reg_e=1e-1)
