...
def compute_barycenter(n, dists, reg=1e-1):
    def _to_prob(a, eps=1e-10):
        a = np.asarray(a, dtype=np.float64)
        a = np.clip(a, 0, None)        # no negative values
        a = a + eps                    # make every entry > 0
        return a / a.sum()
    A = np.vstack([_to_prob(d) for d in dists]).T
    M = ot.utils.dist0(n)
    M /= M.max()
    weights = np.ones(len(dists)) / len(dists)
    WB = ot.bregman.barycenter(A, M, reg, weights, numItermax=1000000)
    return WB
...
class Agent:
    ...
    def project_to_global(self, global_bary):
        ...
        def project(dataset, holder):
            for i, t in tqdm(enumerate(dataset), desc=f"Agent {self.id} projecting", leave=False):
                ...
                img_t = (
                    torch.from_numpy(transported.astype(np.float32))
                    .reshape(32, 32, 3)
                    .permute(2, 0 ,1)
                    )
    ...
    def local_test(self):
        ...
        self.acc_test_tracker.append(acc)
        return acc
...
class Server:
    ...
    def test_global(self, test_loader):
        ...
        self.acc_tracker.append(correct/total)

if __name__ == "__main__":
    parser = argparse.ArgumentParser("OT-Preprocessing algorithm.")
    parser.add_argument("--N", type=int, default=100, help="Total number of agents in the network") 
    parser.add_argument("--P", type=int, default=100, help="Number of participating agents per round")
    parser.add_argument("--rounds", type=int, default=1000, help="Total federated training rounds") 
    parser.add_argument("--epochs", type=int, default=10, help="Local epochs per round")
    parser.add_argument("-n", type=int, default=250, help="Number of pixels to subsample") 
    parser.add_argument('--reg', type=float, default=1e-2, help='Entropy regularisation for local/global barycenters')
    ...
    for i in range(N):
        ...
        agents.append(Agent(i, tr, te, device, reg=reg, n=n))
    server = Server(N, device)
    global_test_loader = DataLoader(test_set, batch_size=128)

    # OT alignment
    print('performing ot-alignment')
    local_barys = []
    for a in agents:
        print(f'computing local wb for agent {a.id}')
        local_barys.append(a.compute_local_barycenters())
    print('computing global barycenter')
    global_bary = server.compute_and_broadcast_global_bary(local_barys)
    for a in agents:
        print(f'projecting images for agent {a.id}')
    ...
    acc_local_test = []
    for a in agents:
        acc_local_test.append(a.acc_test_tracker)
    to_save = {
        'local': acc_local_test,
        'global': server.acc_tracker
    }
